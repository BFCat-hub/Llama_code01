...
void opLadj1_cpu(float *vec, float *vec1, float *vec2, float *vec3, long depth, long rows, long cols) {
    for (int x = 0; x < cols; x++) {
        for (int y = 0; y < rows; y++) {
            for (int z = 0; z < depth; z++) {
                ...
                vec[i + cols] = vec1[i + cols] + 0.25 * (vec2[i + cols] + vec2[i] + vec2[i + cols + 1] + vec2[i + 1]) +
                                0.5 * (vec3[i + cols] + vec3[i + cols + 1]);
...
int main() {
    // Test opLadj1_cpu function with a simple example
    long depth = 3;
    long rows = 4;
    long cols = 5;
    ...
    // Call the opLadj1_cpu function
    opLadj1_cpu(vec, vec1, vec2, vec3, depth, rows, cols);

    // Display the results (you may modify this part based on your actual data)
    printf("Results after opLadj1_cpu function:\n");
    for (int i = 0; i < depth * rows * cols; i++) {
        printf("%f ", vec[i]);
    }
