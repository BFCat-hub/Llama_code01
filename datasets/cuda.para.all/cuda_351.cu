...
int main() {
    const long depth = 32;   // Adjust the depth based on your data
    const long rows = 512;   // Adjust the number of rows based on your data
    const long cols = 512;   // Adjust the number of columns based on your data
    ...
    // Launch the kernel
    opLadj1<<<grid_size, block_size>>>(d_vec, d_vec1, d_vec2, d_vec3, depth, rows, cols);
